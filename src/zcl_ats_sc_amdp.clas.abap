CLASS zcl_ats_SC_amdp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_customer_by_id IMPORTING
                                        value(i_bp_id) TYPE zats_SC_EY_dte_id
                                     EXPORTING
    ...
    CLASS-METHODS get_product_mrp IMPORTING
                                    VALUE(i_tax) type i
                                  EXPORTING
                                    VALUE(otab) type zats_SC_tt_product_mrp.

    CLASS-METHODS get_total_sales for table FUNCTION ZATS_SC_TF.
...
CLASS zcl_ats_SC_amdp IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

     zcl_ats_SC_amdp=>get_product_mrp(
       EXPORTING
         i_tax = 18
       IMPORTING
         otab  = data(itab)
...
*
*    zcl_ats_SC_amdp=>get_customer_by_id(
*      EXPORTING
*        i_bp_id = '067AA621F9F71EEF8BA6629CB24D3A4A'
*      IMPORTING
*        e_res   = data(lv_res)
*    ).
*    out->write(
...
*    ).
  ...
  METHOD get_customer_by_id BY DATABASE PROCEDURE FOR HDB LANGUAGE SQLSCRIPT
    ...
    select company_name into e_res from zats_SC_bpa where bp_id = :i_bp_id;

  ENDMETHOD.

  METHOD get_product_mrp BY DATABASE PROCEDURE FOR HDB LANGUAGE SQLSCRIPT
                            options read-only
                            USING zats_SC_product.
*   declare variables
...
*   get all the products in a implicit table (like a internal table in abap)
    lt_prod = select * from zats_SC_product;
...
*   fill the otab for result (like in abap we fill another internal table with data)
  ...
  METHOD get_total_sales by DATABASE FUNCTION FOR HDB LANGUAGE SQLSCRIPT
                        OPTIONS READ-ONLY
                        USING zats_SC_bpa zats_SC_so_hdr zats_SC_so_item
  .
    ...
    return select
     ...
     from zats_SC_bpa as bpa
    INNER join zats_SC_so_hdr as sls
    on bpa.bp_id = sls.buyer
    inner join zats_SC_so_item as item
